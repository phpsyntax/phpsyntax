...
The same listing for any code of your own is one command away: `vendor/bin/phpsyntax dump file.php`, or
`dump -e '<?php ...'` for a snippet, with `--as=expression` for a piece that is not a whole file.
...
interface, which is how `find(ClassLikeNode::class)` collects classes, interfaces, traits, enums and
anonymous classes in one go.
...
```
written                  kind           role       short name         refers
Shop\Billing             Qualified      ClassLike  Billing            false
Shop\Money               Qualified      ClassLike  Money              false
Money                    Unqualified    ClassLike  Money              true
\DateTimeImmutable       FullyQualified ClassLike  DateTimeImmutable  true
string                   Unqualified    ClassLike  string             false
\sprintf                 FullyQualified Function   sprintf            true
DATE_ATOM                Unqualified    Constant   DATE_ATOM          true
...
The role says which table of names a name belongs to, not whether it refers to anything, and the table
shows three names where the two differ. The built-in type `string` is `ClassLike` by its place in the
tree. `static` is a name written as a keyword token, which `isKeyword()` tells you, and where a class is
named it stands for one only where it is written. And a name inside a `use` or `namespace` statement
carries the role of what it imports, `ClassLike` for a plain import, because PHP keeps classes and
namespaces in one table and `use Foo;` introduces both at once. **`isReference()`** answers all of that
at once, the last column: it is `false` for a built-in type, for `self`, `static` and `parent` and for
the name an import or a namespace introduces. Ask it before you resolve a name.
...
In an abstract syntax tree the value is the node, and the way it was written is at best an attribute
beside it that a printer rebuilds the literal from. Here the notation is the token itself:
`$token->text` is `0x1F`, `$base` is 16 and `$value` is 31, so a rule about hexadecimal notation is
three lines and cannot accidentally rewrite decimals.
...
`findArgument('subject', 2)` takes the name of the parameter and its position, counted from zero, and
returns whichever argument feeds it: the one written with that name wherever it stands, otherwise the one in that
...
handing you the text of the expression. The `$name` and `$key` listed under them are the variables
inside `$$name` and `${$key}`, variables of their own that `find()` visits too.

## Try it yourself

1. Read the slot names of a `match` expression or a class with attributes, by pointing `inspect.php` at
   it or with `vendor/bin/phpsyntax dump -e '<?php ...'`.
2. In `find.php`, list every method of the class with its line: `find(MethodNode::class)` plus
   `getStartLine()`.
3. In `literals.php`, add `1_000_000` and `0b1010` to the sample and check that each keeps its
   notation, with the base 10 and 2 beside the value.
