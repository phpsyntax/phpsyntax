...
$if = must($file->findFirst(IfNode::class));
...
//    goes where the policy says; this one is about the line that goes, so it goes too, while
//    the default CommentPolicy::MoveToNextToken would have kept it above the next statement
must($file->findFirst(ExpressionStatementNode::class))->remove(CommentPolicy::Drop);
