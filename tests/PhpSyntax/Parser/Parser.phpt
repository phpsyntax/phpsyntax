...
use PhpSyntax\{Node, ParseException, Parser, Printer, Token, TokenKind};
...
test('syntax errors', function () {
	...
	Assert::exception(fn() => parse('<?php class {}'), ParseException::class, "Unexpected '{', expecting identifier");
	// the expected tokens are named the way PHP names them
	Assert::exception(fn() => parse('<?php $a->;'), ParseException::class, "Unexpected ';', expecting identifier, variable, '{' or '$'");
