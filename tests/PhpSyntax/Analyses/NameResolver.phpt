...
test('a name that refers to no symbol is refused, not read as one of the namespace', function () {
	...
	Assert::exception(
		...
		"'int' refers to no symbol to resolve: it declares one, names a builtin type, or stands for a class only where it is written.",
	);
	Assert::exception(fn() => $resolver->resolveClass($namespace), InvalidArgumentException::class, "'App' refers to no symbol to resolve: %a%");
	Assert::exception(fn() => $resolver->resolveFunction($import), InvalidArgumentException::class, "'Lib\\helper' refers to no symbol to resolve: %a%");
});
