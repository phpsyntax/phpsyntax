...
- A node written into a new place carries the trivia of the place it came from, a clone and one moved out of a subtree without a file alike, so `setEdgeTrivia([], [])` clears them and `withoutEdgeTrivia()` copies a node without them; only `Parser::parseFragment()` gives a node with empty edges. A write may take a node out of the child it releases and out of a subtree that has no file, never out of a live tree and never out of the node doing the write.
