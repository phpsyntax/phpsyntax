...
`namespace` or `use` statement has the role of a class, because PHP keeps namespaces and classes in one table,
and so does `int` in a type position, because a type is written as a name. Ask `isReference()` before you
...
| `Scalar\InterpolatedStringPart` | `Scalar\InterpolatedStringPartNode`, `Scalar\UnquotedStringNode` |
...
| `ConstFetch` named `true`, `false`, `null` | `Scalar\BooleanNode`, `Scalar\NullNode`, with `$value` |
...
- **One declaration, several members.** `public $a, $b;` is one `PropertyNode` with two items, and so is
  ...
  once per item, and has to remember which one it already handed out. `Extractor` in nette/php-generator
  keeps an `SplObjectStorage` for exactly this.
...
`Nette\PhpGenerator\Extractor` reads a PHP file and rebuilds it as a `PhpFile` object, method bodies included.
Its move to PhpSyntax (nette/php-generator, commit `28dc090e`) is a fair sample of the shape of this work:

- Four private methods went away entirely: `getNodeContents()`, `prepareReplacements()`,
  `performReplacements()` and `getNodeStartPos()`. They existed to cut text out of the source by byte offset
