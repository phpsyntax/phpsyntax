...
trivia of the next token. So a line ending among leading trivia is always a blank line or the end of the
line a comment stands on, which is what `setBlankLinesBefore()` counts. Four things sit outside the
rule: `<?php` is leading trivia and never a token; the end-of-file token has only leading trivia, and
carries whatever stands after the last line ending; a token whose own text ends a line (`?>`, the start
of a heredoc) closes its line by itself, and `getTrailingSpace()` answers `null` after it just as it does
after a line ending; and whitespace inside a string interpolation carries `inInterpolation` and is left
alone, because there it can change what the string reads (`"${a}"` against `"${a }"`).
...
and the indentation is not computed by the tool: **`Indentation::infer()`** gives the one the place in
the tree conventionally has, the braces level with the method and the statement one unit deeper. And
**nothing else in the file moves**: the sample keeps its tabs and the file still ends the way it ended.
...
2. Give `whitespace.php` the style `new Style(indent: '    ')` and see that the lines the tool writes
   are indented with spaces while the rest of the file keeps its tabs.
