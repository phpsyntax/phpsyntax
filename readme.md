...
That one line is the whole difference between this and the abstract syntax tree most PHP tooling is built
on, and it is what the library is for: change one thing and the diff contains one thing.
...
result would not read back the same way. A node of the file goes in as a copy, `withoutEdgeTrivia()`,
because a factory refuses one that still stands in the tree:

```php
FunctionCallNode::of(NameNode::fromText('count'), ArgumentListNode::of($value->withoutEdgeTrivia()));
MethodCallNode::of($object->withoutEdgeTrivia(), 'query', ArgumentListNode::of($sql->withoutEdgeTrivia()));
NewNode::of(NameNode::fromText('DateTime'));                     // new DateTime
MethodCallNode::of($parser->parseExpression('new Foo'), 'bar');  // (new Foo)->bar()
CombinedAssignmentNode::of($total->withoutEdgeTrivia(), '+=', $vat->withoutEdgeTrivia());  // $total += $vat
```

▶ Runnable example: [examples/mutation/factories.php](examples/mutation/factories.php)
...
$token->ensureLeadingNewline();      // onto its own line, and then
$token->setBlankLinesBefore(1);      // a blank line above it
...
$file->getIndex()->findNode($start, $end);  // a position from an editor or another tool, back to a node
```
...
- **A concrete tree costs more than an AST.** Every token is an object and carries its trivia, so the tree
  of a file takes several times the memory of an abstract tree of it, and parsing takes about twice as long
  as php-parser needs. That is the price of the round trip, and it is why tools built on this parse a file,
