...
use PhpSyntax\{ParseException, Token, TokenKind, Trivia};
use Tester\Assert;
...
test('lexer errors', function () {
	...
	// a string is refused where it starts, not at the end of the file its parser would reach
	$e = Assert::exception(fn() => tokenize("<?php\n\$a = 'abc;\n"), ParseException::class, 'Unterminated string');
	Assert::type(ParseException::class, $e);
	Assert::same([2, 6, 11], [$e->sourceLine, $e->sourceColumn, $e->sourceOffset]);
	$e = Assert::exception(fn() => tokenize("<?php\n\$a = \"abc {\$b}\n"), ParseException::class, 'Unterminated string');
	Assert::type(ParseException::class, $e);
	Assert::same([2, 6, 11], [$e->sourceLine, $e->sourceColumn, $e->sourceOffset]);
	Assert::exception(fn() => tokenize("<?php\n\$a = `ls"), ParseException::class, 'Unterminated shell command');
	Assert::exception(fn() => tokenize("<?php\n\$a = <<<X\nabc\n"), ParseException::class, 'Unterminated heredoc');
	Assert::exception(fn() => tokenize("<?php\n\$a = <<<'X'\nabc\n"), ParseException::class, 'Unterminated nowdoc');
	// a quote inside an interpolation opens a string of its own
	Assert::count(18, tokenize("<?php \$a = \"{\$b[\"k\"]} `c`\" . `d \$e`;"));
});
