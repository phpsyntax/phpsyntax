...
function buildNodes(array $schema, string $dir): void
{
	...
	foreach ($schema as $class => $definition) {
		...
		if (is_file($file)) {
			...
			$code = replaceOnce($code, '~^\tpublic const Slots = \[[\s\'\w,]*\];$~m', $constant, "$class: the Slots constant");
			$code = replaceOnce($code, '~(?:^\t/\*\*[^\n]*\*/\n)?^\tpublic function __construct\(.*?\n\t\}~ms', $constructor, "$class: the constructor");
			foreach (array_diff($imports, readImports($code)) as $import) {
...
/**
 * The fully qualified names the file imports, whether an import stands alone, in a comma-separated list or in a group.
 * @return list<string>
 */
function readImports(string $code): array
{
	preg_match_all('~^use (?!function |const )([^;]+);$~m', $code, $matches);
	$names = [];
	foreach ($matches[1] as $clause) {
		[$prefix, $items] = preg_match('~^(.+?)\\\\\{(.+)\}$~s', $clause, $m) ? [$m[1] . '\\', $m[2]] : ['', $clause];
		foreach (explode(',', $items) as $item) {
			$names[] = $prefix . preg_replace('~\s+as\s+\w+$~', '', trim($item));
		}
	}
