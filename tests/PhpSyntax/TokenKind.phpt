...
function getKinds(): array
{
	...
	return array_filter($constants, 'is_int'); // the maps of names and of host constants are not kinds
}
...
test('every kind has a name and no name stands for two kinds', function () {
	Assert::same(getKinds(), array_flip(TokenKind::Names));
});
