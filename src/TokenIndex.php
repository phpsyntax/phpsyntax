 ...
 * rebuilt: the tokens of adopted and released children move at the next query, the numbering and the lines
 * after a change are brought up to date as far as the queries reach, and offsets and columns are recomputed
 * from scratch.
 */
		...
		foreach ($node->getChildren() as $child) {
			if ($child->parent !== $node) { // a write took it and left the hole behind: it stands twice
				throw new \LogicException(
					($child instanceof Token ? "Token '$child->text'" : $child::class)
