...
that measures columns in a loop should read them before it starts editing, or edit from the end of the
file backwards, so that the positions read up front stay true for everything still ahead.

Meanwhile `$token->originalLine` and `$token->originalOffset` never move: they are where the token was in the
...
**`getLineWidth()` measures a line the way an editor does**: every tab reaches to the next tab stop of
the style, wherever on the line it stands, and trailing whitespace does not count, which is why the
widths above differ from `strlen()`. A "line too long" rule is then a `startsLine()` filter and a
...
bytes 66 to 82, another parser gives the offsets of its own node. **`findNode()`** of the file's index
brings such a position to the tree: the outermost node of the class you ask for whose text stands
...
the rename the call ends a byte later, and **`contains()`** tells whether a token belongs to this file
...
1. In `lines.php`, insert `$this->sum ??= 0;` before the `return` instead of adding blank lines, with
   `$return->parent->insert(0, ...)`, and check that the return moves to line 7 while `originalLine`
   stays 6.
