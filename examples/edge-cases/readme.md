...
the space in "{$a }" is marked: true
and not reformatted: Token '$a' is inside string interpolation; its whitespace cannot be changed.
```
...
- **`<?php` is not a token at all.** It is `OpenTag` trivia in front of the first token, carrying its own
  whitespace, which is why a rewrite cannot lose it, and why a copy of a node from elsewhere in the file
  is taken with `withoutEdgeTrivia()`, not with `clone`, or it brings a second `<?php` along.
...
- **Whitespace inside an interpolation is code inside a string**, and a formatter must not touch it:
  `"${a}"` is the variable `$a`, while `"${a }"` is the variable the constant `a` names. Such trivia
  carry `inInterpolation`, and the whitespace helpers refuse to reformat them.
