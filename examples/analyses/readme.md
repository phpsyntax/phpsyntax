# PHP name resolution and scope for static analysis: what a name refers to and where a node stands
...
`FunctionLikeNode`, so `$parameters` and `$returnType` are there whether it is a function, a method, a
...
1. In `name-resolution.php`, add `use Shop\Money as Cash;` to the imports and a parameter `Cash $tip` to
   `total()`: the table resolves `Cash` to `Shop\Money`, and `Shop\Money` is now written as `Cash`, the
   shorter of the two aliases.
