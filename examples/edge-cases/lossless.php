 ...
 *               `Style::detectEol()`, `Token::isSemicolon()`, `Trivia::$inInterpolation`
...
$html = must($file->findFirst(InlineHtmlNode::class));
...
$closeTag = must($file->findFirst(EmptyStatementNode::class))->semicolon;
...
// the space inside an interpolation is code inside a string, and the whitespace helpers leave it alone
$file = $parser->parse('<?php $s = "{$a }";');
$a = must(must($file->findFirst(VariableNode::class, fn(VariableNode $variable) => $variable->plainName === 'a'))->getFirstToken());
echo 'the space in "{$a }" is marked: ', var_export($a->trailingTrivia[0]->inInterpolation, return: true), "\n";
try {
	$a->setTrailingSpace('');
} catch (LogicException $e) {
	echo 'and not reformatted: ', $e->getMessage(), "\n";
}
