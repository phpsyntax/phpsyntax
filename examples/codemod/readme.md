...
- why a factory takes a copy of a node, and why the node leaves its whitespace behind
...
them directly. They go in as `withoutEdgeTrivia()` copies, because a factory refuses a node that still
stands in the file, and refuses it before it changes anything. The whitespace on their edges the factory
drops by itself: a node carries the whitespace of the place it came from, and the new place already has
its own, in the tokens around it. That is the thing to remember from this chapter: **whitespace belongs
to a place, not to a node.**
...
- Returning `StopTraversal` ends the walk, from `enter` as well as from `leave`.
- The two answers are the integers 1 and 2, and anything else leaves the walk alone. So an arrow
  function such as `fn($node) => $this->count++` steers the walk without meaning to, once the counter
  reaches 1; a callback that steers nothing gets a body that returns nothing.
...
2. Run the codemod over a real file of your own: read it, parse it, rewrite it, and write the result
   back only when something changed, the way [mutation/](../mutation#writing-the-result-back) does it.
