...
use PhpSyntax\{ParseException, Parser, TokenKind};
use Tester\Assert;
...
test('parseExpression: detached, without positions, with empty trivia on the edges', function () {
	...
	$e = Assert::exception(fn() => (new Parser)->parseExpression("\$a\n+"), ParseException::class, 'Unexpected end of the fragment');
	Assert::type(ParseException::class, $e);
	Assert::same([2, 2, 4], [$e->sourceLine, $e->sourceColumn, $e->sourceOffset]);
	Assert::exception(fn() => (new Parser)->parseExpression('[1, 2'), ParseException::class, "Unexpected end of the fragment, expecting ']'");
	$e = Assert::exception(fn() => (new Parser)->parseExpression('f(1 2)'), ParseException::class, "Unexpected '2', expecting ')'");
	Assert::type(ParseException::class, $e);
	Assert::same([1, 5, 4], [$e->sourceLine, $e->sourceColumn, $e->sourceOffset]);
	$e = Assert::exception(fn() => (new Parser)->parseExpression('"abc'), ParseException::class, 'Unterminated string');
	Assert::type(ParseException::class, $e);
